...
#### 5. `modules/jenkins`
Цей модуль розгортає Jenkins у Kubernetes кластер за допомогою офіційного Helm-чарту та автоматично налаштовує його для CI/CD.
- **Helm Release:** Встановлює Jenkins в окремий неймспейс `jenkins`.
- **Persistent Storage:** Створює `PersistentVolumeClaim` для збереження даних Jenkins.
- **LoadBalancer Service:** Забезпечує зовнішній доступ до Jenkins.
- **JCasC (Jenkins Configuration as Code):** Автоматично налаштовує:
    - **Креденшели:** Додає GitHub PAT для доступу до репозиторіїв.
    - **Seed Job:** Створює `seed-job`, який сканує вказаний Git-репозиторій і автоматично створює CI-пайплайн (`goit-django-docker`) на основі `Jenkinsfile`.
- **IAM & Service Account:** Створює необхідну IAM-роль та Service Account (`jenkins-sa`) для інтеграції з AWS ECR за допомогою IRSA, що дозволяє Kaniko безпечно збирати та пушити Docker-образи.
...
### ❗️ Важливе попередження про безпеку

Перед застосуванням конфігурації вам необхідно оновити файл `lesson-8-9/modules/jenkins/values.yaml`:

1.  Знайдіть секцію `JCasC.configScripts.credentials`.
2.  Замініть `YOUR_GITHUB_PAT_HERE` на ваш справжній GitHub Personal Access Token.

**НІКОЛИ не коммітьте цей файл зі справжнім токеном у ваш Git-репозиторій!** 
...
### 5. Автоматизований CI/CD Процес

Завдяки конфігурації через JCasC, Jenkins готовий до роботи одразу після встановлення.

1.  **Seed Job:** Автоматично створена задача `seed-job` сканує ваш Git-репозиторій (вказаний у `values.yaml`).
2.  **Створення Пайплайну:** На основі знайденого `Jenkinsfile`, `seed-job` створює новий пайплайн з назвою `goit-django-docker`.
3.  **Виконання Пайплайну:** Цей пайплайн використовує **Kaniko** для збірки Docker-образу з вашого `Dockerfile` та завантажує його у ваш Amazon ECR репозиторій. Весь процес відбувається всередині Kubernetes без прямого доступу до Docker-демона.

Ви можете запустити `seed-job` вручну, щоб створити пайплайн, а потім запустити `goit-django-docker` для збірки вашого проєкту.
